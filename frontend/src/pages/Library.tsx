...
const Library = () => {
  ...
  const handleUpdateAvatar = (newAvatarUrl: string) => {
    setUser((prev: any) => prev ? { ...prev, avatarUrl: newAvatarUrl } : prev);
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/documents/${editingId}/title`, {
        method: "PATCH",
      ...
      setBooks((prev) =>
        prev.map((book) => book.id === editingId ? { ...book, title: trimmed } : book)
      );
      setEditingId(null);
    } catch (error) {
      console.error(error);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Erro ao apagar livro");
  ...
  const handleFileUpload = async (file: File) => {
    try {
      ...
      if (!response.ok) throw new Error("Erro ao enviar PDF para o backend");
  ...
  const moveBookToTop = async (id: string) => {
    try {
      await fetch(`${baseUrl}/documents/${id}/open`, { method: "PATCH" });
      setBooks((prev) => {
  ...
  const toggleFavorite = async (id: string, currentFavorite: boolean) => {
    ...
    try {
      const response = await fetch(`${baseUrl}/documents/${id}/favorite`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteAccount = async () => {
    const confirmed = window.confirm("Tem certeza que deseja excluir sua conta? Essa ação apagará seus livros, progresso, favoritos e configurações.");
    if (!confirmed) return;
      ...
      {isUploading && (
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center">
          <div className="bg-card border border-border rounded-2xl shadow-smooth px-8 py-6 text-center space-y-3">
            ...
            <div>
              <p className="font-semibold text-foreground">Processando PDF...</p>
      ...
      <main className="max-w-5xl mx-auto px-6 py-12 space-y-8">
        <div className="flex items-center justify-between">
          ...
          {libraryBooks.length > 0 && (
            <button onClick={() => setShowUpload(true)} className="flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground rounded-full font-medium hover:opacity-90 transition-smooth">
        ...
        {showUpload && (
          <div
            ...
            style={{ borderColor: isUploadHovered ? '#87ceeb' : 'rgba(97,218,251,0.67)' }}
            onMouseEnter={() => setIsUploadHovered(true)}
            onMouseLeave={() => setIsUploadHovered(false)}
          >
            <button onClick={() => setShowUpload(false)} className="absolute top-4 right-4 p-2 hover:bg-secondary rounded-full transition-smooth z-10">
        ...
        {isLoadingBooks ? (
          <div className="text-center py-20">
            <div className="w-10 h-10 border-4 border-primary/30 border-t-primary rounded-full animate-spin mx-auto" />
            <p className="text-muted-foreground mt-4">Carregando biblioteca...</p>
          </div>
        ) : libraryBooks.length === 0 ? (
          <div className="text-center py-24 space-y-4">
            <BookOpen size={48} className="mx-auto text-muted-foreground/40" />
            <p className="text-lg text-muted-foreground">Sua estante está limpa. Que tal começar um novo livro hoje?</p>
            <button onClick={() => setShowUpload(true)} className="px-6 py-3 bg-primary text-primary-foreground rounded-full font-medium hover:opacity-90 transition-smooth">
          ...
          <div className="grid gap-4">
            {libraryBooks.map((book) => (
              <div key={book.id} className="flex items-center gap-3">
                ...
                >
                  <Star size={22} className={book.favorite ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"} />
                </button>

                <div className="flex-1 bg-card rounded-2xl p-6 flex items-center justify-between border-2 border-transparent hover:border-primary/40 transition-smooth">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <div className="w-32 h-1.5 bg-secondary rounded-full overflow-hidden">
                          <div className="h-full bg-primary rounded-full transition-smooth" style={{ width: `${book.progress ?? 0}%` }} />
                        </div>
                        <span className="text-xs text-muted-foreground tabular-nums">{book.progress ?? 0}%</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 ml-auto">
                    {editingId === book.id ? (
                      <button onClick={saveEdit} className="p-2 rounded-full text-muted-foreground hover:text-primary hover:bg-primary/10 transition-smooth">
                        <Check size={18} />
                      </button>
                    ) : (
                      <button onClick={() => startEdit(book)} className="p-2 rounded-full text-muted-foreground hover:text-primary hover:bg-primary/10 transition-smooth">
                        <Pencil size={18} />
                      </button>
                    )}
                    <button onClick={() => handleDelete(book.id)} className="p-2 rounded-full text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-smooth">
                      <Trash2 size={18} />
                    </button>
                    <Link to={`/reader/${book.id}`} onClick={() => moveBookToTop(book.id)} className="px-5 py-2.5 bg-foreground text-background rounded-full text-sm font-medium hover:opacity-90 transition-smooth">
                      Continuar leitura
                    </Link>
                  </div>
                </div>
